...
class ContentClassifier:
   ...
   def _log_statistics(self):
       """
       Выводит статистику по классификации
       """
       try:
           if not self.db_manager:
               return
           
           # Получаем статистику по категориям для релевантных постов
           ...
           logger.info("=== Статистика классификации релевантных постов ===")
           
           # Сортируем категории по количеству постов
           sorted_categories = sorted(stats.items(), key=lambda x: x[1]['total'], reverse=True)
           
           for category, data in sorted_categories:
               logger.info(f"{category}: {data['total']} постов")
               
               # Выводим топ-3 подкатегории
